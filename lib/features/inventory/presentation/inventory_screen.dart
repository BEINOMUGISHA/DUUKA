...
class _InventoryScreenState extends ConsumerState<InventoryScreen> {
  ...
  Future<void> _addCategory(TextEditingController categoryController) async {
  ...
  void _showAddEditProductDialog([LocalProductData? existing]) {
    ...
    final sellCtrl = TextEditingController(
        text: existing != null ? existing.sellPrice.toInt().toString() : '');
    final wholesaleCtrl = TextEditingController(
        text: existing?.wholesalePrice != null
            ? existing!.wholesalePrice!.toInt().toString()
            : '');
    final stockCtrl = TextEditingController(
        text: existing != null ? existing.currentStock.toInt().toString() : '');
    final minStockCtrl = TextEditingController(
        text:
            existing != null ? existing.minStockLevel.toInt().toString() : '5');
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                controller: nameCtrl,
                decoration:
                    InputDecoration(labelText: '${ref.tr('product_name')} *'),
              ),
              ...
              Row(
                children: [
                  ...
                  Expanded(
                    child: DropdownButtonFormField<String>(
                      initialValue: ['pcs', 'bag', 'pkt', 'bottle', 'kg', 'tin']
                              .contains(unitCtrl.text)
                          ? unitCtrl.text
                          : 'pcs',
                      decoration:
                          InputDecoration(labelText: '${ref.tr('unit')} *'),
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                          labelText: '${ref.tr('cost_price')} (UGX) *'),
                    ),
                  ),
                  const SizedBox(width: 10),
                  Expanded(
                    child: TextField(
                      ...
                      keyboardType: TextInputType.number,
                      decoration: InputDecoration(
                          labelText: '${ref.tr('selling_price')} (UGX) *'),
                    ),
                  ),
                ],
              ),
                ...
                children: [
                  Expanded(
                    child: TextField(
                      ...
                      keyboardType: TextInputType.number,
                      decoration: InputDecoration(
                          labelText: '${ref.tr('current_stock')} *'),
                    ),
                  ),
                  const SizedBox(width: 10),
                  Expanded(
                    child: TextField(
                      ...
                      keyboardType: TextInputType.number,
                      decoration: InputDecoration(
                          labelText: '${ref.tr('min_stock_alert')} *'),
                    ),
                  ),
                ],
              ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    ...
                    final wholesale =
                        double.tryParse(wholesaleCtrl.text.trim());
                    final stock = double.tryParse(stockCtrl.text.trim()) ?? 0;
                    final minStock =
                        double.tryParse(minStockCtrl.text.trim()) ?? 5;
                    ...
                    if (name.isEmpty || sell <= 0) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content: Text(
                                'Please enter a valid product name and selling price')),
                      );
                      return;
                    }
  ...
  void _showRestockDialog(LocalProductData product) {
    ...
    final costCtrl =
        TextEditingController(text: product.costPrice.toInt().toString());
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () {
                    ...
                    final unitCost = double.tryParse(costCtrl.text.trim()) ??
                        product.costPrice;
                    final session = ref.read(authProvider);
                    ...
                    if (qty <= 0) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content:
                                Text('Please enter a valid received quantity')),
                      );
                      return;
                    }
                    ...
                    ref.read(productsProvider.notifier).restockProduct(
                          ...
                          supplierName: supplierCtrl.text.isNotEmpty
                              ? supplierCtrl.text
                              : null,
                          notes:
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content: Text(
                              'Restocked ${qty.toInt()} ${product.unit} of ${product.name}')),
                    );
                  },
  ...
  void _confirmDeleteProduct(LocalProductData product) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'Are you sure you want to remove "${product.name}" from active inventory?'),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    final totalStockValue = products.fold<double>(
        0, (sum, p) => sum + (p.currentStock * p.sellPrice));
    final totalCostValue = products.fold<double>(
        0, (sum, p) => sum + (p.currentStock * p.costPrice));
    final lowStockCount =
